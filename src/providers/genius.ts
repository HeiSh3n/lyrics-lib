import { GENIUS_API_BASE, GENIUS_CLIENT_HEADER } from '../config/constants.js';
import { KeyError } from '../errors/KeyError.js';
import { RequestError, type RequestErrorOptions } from '../errors/RequestError.js';
...
function pickHit(hits: GeniusSearchHit[], artist?: string): GeniusSearchHit | null {
  const songs = hits.filter((h) => h.type === 'song' && h.result?.url);
  if (songs.length === 0) return null;
  const first = songs[0] ?? null;
  if (!artist) return first;
  const needle = artist.trim().toLowerCase();
  const matched = songs.find((h) =>
    (h.result.primary_artist?.name ?? '').toLowerCase().includes(needle),
  );
 ...
 * HTML.
 */
function toCodePoint(n: number): string | null {
...
const GENIUS_SECTION_RE = new RegExp(
  `^\\[(?:${GENIUS_SECTION_LABELS.join('|')})(?:\\s+\\d+)?(?:\\s*:\\s*[^\\]\\n]*)?\\]`,
  'im',
);
 ...
 */
function stripGeniusMetadata(text: string): string {
...
async function safeFetch(
  url: string,
  init: RequestInit,
  ctx: { track: string; artist?: string; safeUrl: string },
): Promise<Response> {
  try {
    return await fetch(url, init);
  } catch (cause) {
    const opts: RequestErrorOptions = { cause, url: ctx.safeUrl, track: ctx.track };
    if (ctx.artist !== undefined) opts.artist = ctx.artist;
    throw new RequestError(`Genius network request failed: ${ctx.safeUrl}`, opts);
  }
}
 ...
 */
export function createGeniusProvider(options: GeniusProviderOptions): LyricsProvider {
  const apiKey = options?.apiKey?.trim();
  if (!apiKey) {
    throw new KeyError(
      'Genius API key is required. Pass `{ apiKey }` to createGeniusProvider().',
      { provider: 'genius', reason: 'missing' },
    );
  }
  return {
    name: 'genius',
    ...
    async fetchLyrics({ title, artist }: GetLyricsOptions): Promise<string | null> {
      ...
      const searchCtx: { track: string; artist?: string; safeUrl: string } = {
        track: title,
        safeUrl: searchSafeUrl,
      };
      if (artist !== undefined) searchCtx.artist = artist;
      ...
      const searchRes = await safeFetch(
        searchUrl,
        {
          headers: {
            ...
            'User-Agent': GENIUS_CLIENT_HEADER,
          },
        },
        searchCtx,
      );
      if (searchRes.status === 401 || searchRes.status === 403) {
        throw new KeyError(
          `Genius rejected the API key (HTTP ${searchRes.status}). The token is wrong, revoked, or out of quota.`,
          { provider: 'genius', reason: 'rejected', status: searchRes.status },
        );
      }
      if (!searchRes.ok) {
        const opts: RequestErrorOptions = {
          status: searchRes.status,
          url: searchSafeUrl,
          track: title,
        };
        if (artist !== undefined) opts.artist = artist;
        const retryAfter = parseRetryAfter(searchRes);
        if (retryAfter !== undefined) opts.retryAfter = retryAfter;
        throw new RequestError(`Genius search failed (HTTP ${searchRes.status})`, opts);
      }

      const data = (await searchRes.json()) as GeniusSearchResponse;
      ...
      const pageCtx: { track: string; artist?: string; safeUrl: string } = {
        track: title,
        safeUrl: pageSafeUrl,
      };
      if (artist !== undefined) pageCtx.artist = artist;
      ...
      if (!pageRes.ok) {
        ...
        if (retryAfter !== undefined) opts.retryAfter = retryAfter;
        throw new RequestError(`Genius page fetch failed (HTTP ${pageRes.status})`, opts);
      }
...
export const geniusProvider: LyricsProvider = {
  ...
  async fetchLyrics(options: GetLyricsOptions): Promise<string | null> {
    const apiKey = readEnvKey();
    if (!apiKey) {
      throw new KeyError(
        'Genius API key not configured. Set GENIUS_API_KEY in the environment or call createGeniusProvider({ apiKey }).',
        { provider: 'genius', reason: 'missing' },
      );
    }
