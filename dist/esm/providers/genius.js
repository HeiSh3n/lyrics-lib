 ...
 */
export function createGeniusProvider(options) {
    const apiKey = options?.apiKey?.trim();
    if (!apiKey) {
        throw new KeyError('Genius API key is required. Pass `{ apiKey }` to createGeniusProvider().', { provider: 'genius', reason: 'missing' });
    }
    return {
        name: 'genius',
        async fetchLyrics({ title, artist }) {
            ...
            if (searchRes.status === 401 || searchRes.status === 403) {
                throw new KeyError(`Genius rejected the API key (HTTP ${searchRes.status}). The token is wrong, revoked, or out of quota.`, { provider: 'genius', reason: 'rejected', status: searchRes.status });
            }
            if (!searchRes.ok) {
                const opts = {
...
export const geniusProvider = {
    ...
    async fetchLyrics(options) {
        ...
        if (!apiKey) {
            throw new KeyError('Genius API key not configured. Set GENIUS_API_KEY in the environment or call createGeniusProvider({ apiKey }).', { provider: 'genius', reason: 'missing' });
        }
